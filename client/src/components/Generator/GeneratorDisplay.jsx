...
function GeneratorDisplay() {
  ...
  // Ejemplo de 3 textos de 50 caracteres cada uno
  const texts = [
    "Discover unexpected career opportunities in the travel industry.",
    "Gain valuable cultural insights and personal growth from experiences.",
    "Build a diverse network of international friends and connections.",
  ];
        ...
        <div className='content-header sm:h-auto xsm:h-auto sm:max-h-[150px]'>
          ...
          <div className='aquivamos xsm:flex-col sm:flex-col xsm:w-full sm:w-full md:flex lg:flex xl:flex items-center justify-between xl:mt-20 lg:mt-10 md:mt-5 sm:mt-3  gap-11 sm:h-screen xsm:h-screen' style={{ height: `calc(100vh - ${50 * 4}px)` }}>
            ...
            <div className='left xsm:w-full sm:w-full xsm:order-2 sm:order-2 md:order-1 lg:order-1 xl:order-1'>
              <InputSection 
              handlerClick={handleGenerateScenarios}
              />
            </div>
          </div>
      </div>
